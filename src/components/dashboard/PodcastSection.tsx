...
export const PodcastSection = ({
  ...
  return <Card className="shadow-card border-0 h-full rounded-3xl">
      <CardHeader>
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <CardTitle className="flex items-center gap-2 cursor-help">
                    ...
                    {isCurrentWeek && <Badge variant="secondary" className="bg-gradient-primary text-white bg-blue-500">
                        New
                      </Badge>}
                  </CardTitle>
                </TooltipTrigger>
      ...
      <CardContent className="space-y-6 bg-white rounded-3xl">
        ...
        <div className={`${isCurrentWeek ? 'bg-gradient-podcast' : 'bg-gradient-to-l from-slate-300 to-slate-400'} rounded-2xl p-6 py-[19px] px-[16px]`}>
          <div className="flex items-center gap-3 mb-4">
            <Button size="icon" className="rounded-full bg-white/20 hover:bg-white/30 text-white" onClick={() => setIsPlaying(!isPlaying)}>
        ...
        <div className="border-t pt-4 px-1">
          ...
          <div className="space-y-1">
            {recentEpisodes.map((episode, index) => <div key={index} className="flex items-center justify-between py-1.5 hover:bg-accent/50 rounded-2xl px-2 -mx-2 cursor-pointer transition-colors" onClick={() => {
            if (onWeekChange) {
              const episodeWeek = startOfWeek(subWeeks(currentWeek, index + 1), {
                weekStartsOn: 1
              });
